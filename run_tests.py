...
for fname in ["main_unified.py", "non_fsb_programs.py", "sport_marketing.py", "pdf_template.py"]:
    try:
        ...
        check(len(chk.dupes) == 0, f"{fname}: no duplicate keys", "; ".join(chk.dupes[:3]))
...
for prog in sorted(nfp.ALL_NON_FSB_PROGRAMS):
    try:
        ...
        for key, val in req.items():
            if key in helpers.SKIP_KEYS:
                continue
            if isinstance(val, list):
                for item in val:
                    if not isinstance(item, str):
                        continue
                    if "xxx" in item.lower() or item.count("X") >= 3:
                        continue
                    if not COURSE_RE.match(item.strip()):
                        issues.append(f"{prog}.{key}[]: '{item}'")
            elif isinstance(val, dict):
                if "credits" not in val:
                    continue
...
CATALOG_MINORS = [
    ...
    "Christian Spiritual Formation Minor",
    ...
    "Music Minor",
    "Nutrition Minor",
    "Peace and Conflict Transformation Minor",
    "Philosophy Minor",
    "Physics Minor",
    ...
    "Sport and Recreational Leadership Minor",
    "Statistics Minor",
    "Theatre Minor",
...
missing = [
    cm
    for cm in CATALOG_MINORS
    if not any(norm_m(cm) in norm_m(sn) or norm_m(sn) in norm_m(cm) for sn in system_minor_names)
]
check(
    len(missing) == 0,
...
TEST_CASES = [
    ...
    },
    {
        ...
        "catalog": "2022-23",
        "satisfied": ["BIBL 2000", "BIBL 2050", "RLGN 2000", "RLGN 2130", "RLGN 3040", "RLGN 3060"],
    ...
    },
    {
        ...
        "catalog": "2022-23",
        "satisfied": ["MUSC 1010", "MUSC 1020", "MUSC 1030", "MUSC 1040", "MUED 2470", "EDUC 2100"],
        "not_satisfied": [],
        "current": [],
        "choose_row_counts": {},
    },
...
for tc in TEST_CASES:
    ...
    try:
        ...
        rows = helpers._build_major_rows(
            req, courses, sm, concentration=tc.get("concentration", "")
        )

        # No duplicate row IDs
        ids = [r["id"] for r in rows]
        dupes = [id for id in set(ids) if ids.count(id) > 1]
        check(len(dupes) == 0, f"{name}: no duplicate rows", f"duplicates: {dupes[:5]}")

        def is_satisfied(code):
            n = sm.norm(code.replace(" ", "_"))
            return any(
                r["course"] and r["course"]["code"] == n and r["status"] == "Satisfied"
                for r in rows
            )
        ...
        def is_current(code):
            n = sm.norm(code.replace(" ", "_"))
            return any(
                r["course"] and r["course"]["code"] == n and r["status"] in ("Current", "Scheduled")
                for r in rows
            )
        ...
        for group_key, expected_n in tc.get("choose_row_counts", {}).items():
            group_rows = [r for r in rows if r["id"].lower().startswith(group_key.lower())]
            check(
                len(group_rows) == expected_n,
        ...
        if tc.get("conc_must_exclude_core"):
            core_codes = {sm.norm(c.replace(" ", "_")) for c in req.get("required_core", [])}
        ...
        if minor_key:
            ...
            check(
                len(minor_dup) == 0,
                f"{name} minor: no duplicate rows",
...
for major_key, engine_path, engine_name, has_minor in FSB_TESTS:
    ...
    try:
        ...
        check("mr" in res and "la" in res, f"FSB {major_key}: audit() returns mr and la rows")
    except Exception as e:
