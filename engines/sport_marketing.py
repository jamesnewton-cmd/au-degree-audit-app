        ...
        # STATUS: COURSE-CODE = current|scheduled|satisfied
            ...
            try:
                ...
                if mapped and code in cm:
                    ...
                    if mapped == "grade posted" and cm[code]["grade"].upper() in (
                        "",
                        "W",
                        "DRP",
                        "NC",
...
ATHLETIC_COACHING_MINOR = [
    ...
    (
        "ATRG_1530",
        "ATRG-1530 or EXSC-4010 Strength & Conditioning",
        ["ATRG_1530", "EXSC_4010"],
        3,
        "choose one",
    ),
    (
        "PEHS_2340",
        "PEHS-2340 or PEHS-3340 Psychology/Sociology of Sport",
        ["PEHS_2340", "PEHS_3340"],
        3,
        "choose one",
    ),
...
LA_ROWS = [
    ...
    (
        ...
        "F3 ENGL-1120 Writing II — ENGL-1120 (or HNRS-2110)",
        ["ENGL_1120", "HNRS_2110"],
        3,
    ),
    (
    ...
    (
        ...
        ["BSNS_1050", "BSNS_4110", "BSNS_4330", "BSNS_4560", "BSNS_4800"],
        2,
    ),
    (
        "WI",
...
MINOR_DEFS = {
    ...
    "sport_marketing_minor": {
        ...
        "rows": [
            ...
            (
                "ELEC",
                "Sport Mktg Elective — 6 hrs from approved list",
                ["BSNS_3210", "BSNS_3220", "BSNS_3550", "BSNS_4400", "BSNS_4550", "BSNS_4800"],
                6,
                "elective",
            ),
        ],
    ...
    "accounting_minor": {
        ...
        "rows": [
            ...
            (
                "ELEC",
                "Accounting Elective — 9 hrs from ACCT 3000+",
                [
                    "ACCT_3010",
                    "ACCT_3020",
                    "ACCT_3110",
    ...
    "management_minor": {
        ...
        "rows": [
            ...
            (
                "ELEC",
                "Management Elective — 9 hrs from approved list",
                ["BSNS_3240", "BSNS_3550", "BSNS_4050", "BSNS_4240", "BSNS_4250", "BSNS_4310"],
                9,
                "elective",
            ),
        ],
    ...
    "marketing_minor": {
        ...
        "rows": [
            ...
            (
                "ELEC",
                "Marketing Elective — 9 hrs from approved list",
                [
                    "BSNS_3210",
                    "BSNS_3220",
                    "BSNS_3130",
    ...
    "finance_minor": {
        ...
        "rows": [
            ...
            (
                "ELEC",
                "Finance Elective — 12 hrs from approved list",
                ["BSNS_3510", "BSNS_4050", "BSNS_4120", "BSNS_4310", "BSNS_4310_23"],
                12,
                "elective",
            ),
        ],
    ...
    "global_business_minor": {
        ...
        "rows": [
            ...
            (
                "ELEC",
                "Global Business Elective — 12 hrs from approved list",
                ["BSNS_3400", "BSNS_3550", "BSNS_4050", "BSNS_4240", "BSNS_4400"],
                12,
                "elective",
            ),
        ],
...
def _run_minor_rows(courses, rows, elec_pool, elec_needed, find, status_of, elec_dept_prefix=None):
    ...
    for rid, label, opts, dcr, note in rows:
        if note == "elective":
            ...
            elec_done = [
                c for c in courses if c["code"] in pool and done(c) and c["code"] not in core_used
            ]
            elec_ip = [
                c
                for c in courses
                if c["code"] in pool and (ip(c) or sched(c)) and c["code"] not in core_used
            ]
...
def build(res, student_name, major_label, out, exceptions=""):
    ...
    if minor_rows := res.get("minor_rows"):
        ...
        res.setdefault(
            "minor_name",
            FSB_MINORS.get(minor_key, {}).get("name", "Minor") if minor_key else "Minor",
        )
...
def build_la_rows_for_non_fsb(courses, catalog_year, major_key=""):
    ...
    if catalog_year == "2025-26":
        ...
        if isinstance(f2_dcr, dict):
            f2_dcr = f2_dcr.get(yr, 3)
        f2_c = find_any(f2_opts + map_by_area.get("F2", []))
        la.append(
            make_row(
                "F2",
                None,
                f2_def.get("label", "F2 Civil Discourse & Critical Reasoning"),
                f2_dcr,
                f2_c,
                status_of(f2_c),
            )
        ...
        f3_1_opts = [c for c in f3_req if "1110" in c or "1100" in c or "HNRS" in c] or [
            "ENGL 1110",
            "ENGL 1100",
            "HNRS 2110",
        ]
        ...
        hnrs_2110_used_for_f3 = (
            f3_1_c is not None
            and "HNRS" in f3_1_c.get("raw", "").upper()
            and "2110" in f3_1_c.get("raw", "")
        )
        ...
        la.append(
            make_row(
                "F3",
                None,
                "F3 Writing I — ENGL-1110 (or ENGL-1100 / HNRS-2110)",
                3,
                f3_1_c,
                status_of(f3_1_c),
            )
        ...
        la.append(
            make_row(
                "F3", None, "F3 Writing II — ENGL-1120 (or HNRS-2110)", 3, f3_2_c, status_of(f3_2_c)
            )
        )
        ...
        if isinstance(f5_dcr, dict):
            f5_dcr = f5_dcr.get(yr, 3)
        ...
        if isinstance(f7_dcr, dict):
            f7_dcr = f7_dcr.get(yr, 2)
        ...
        for wkey in ["W1", "W2", "W3", "W5", "W6", "W7"]:
            ...
            if isinstance(w_dcr, dict):
                w_dcr = w_dcr.get(yr, 3)
        ...
        if w8_courses or map_by_area.get("W8"):
            w8_pool = list(w8_courses or []) + list(map_by_area.get("W8", []))
            # Apply POSC-2840 two-semester rule
            if "POSC_2840" in w8_pool:
                posc_2840_earned = sum(
                    c.get("cr", 0) for c in courses
                    if norm(c.get("code", "")) == "POSC_2840" and done(c)
                )
                posc_2840_ip = sum(
                    c.get("cr", 0) for c in courses
                    if norm(c.get("code", "")) == "POSC_2840" and ip(c)
                )
                # Remove POSC_2840 from pool unless student has earned >=6 cr
                # or has enough in-progress to reach 6 cr
                if posc_2840_earned < 6 and (posc_2840_earned + posc_2840_ip) < 6:
                    w8_pool = [x for x in w8_pool if norm(x) != "POSC_2840"]
            w8_c = find_any(w8_pool)
            la.append(
        ...
        la.append(
            make_row(
                ...
                3,
                wi_c2,
                status_of(wi_c2) if wi_c2 else "Not Satisfied",
            )
        )
...
if __name__ == "__main__":
    ...
    build(
        ...
        "Nolan Netter",
        "Sport Marketing",
        "/mnt/user-data/outputs/Nolan_Netter_Sport_Marketing_Audit.pdf",
    )
# ── WALK ELIGIBILITY ─────────────────────────────────────────────
...
def eligible_to_walk(res):
    ...
    for row in res.get("la", []):
        ...
        action = (row.get("recommended_action") or row.get("note") or "").lower()
