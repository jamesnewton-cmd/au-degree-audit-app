...
from engines.sport_marketing import (
    parse_csv, apply_exceptions, cmap, norm, best,
    done, ip, sched, xfer, drop,
    status_of, grade_disp, cr_disp,
    build_la_rows_for_non_fsb,
    MINOR_DEFS, BUS_CORE, BUS_CORE_2526,
    LA_ROWS, MATH_OPTS,
)
...
def audit(courses, minor_key=None):
    ...
    def find(opts): return best(courses, opts)
    ...
    for row in LA_ROWS:
        ...
        if area == 'W8':
            ...
            wi_upper = ['BSNS_4910', 'BSNS_3120', 'BSNS_4110', 'ENGL_3110',
                        'ENGL_3190', 'ENGL_3580', 'COMM_3130', 'HIST_3260']
            c = None; s = 'Not Satisfied'
    ...
    for course_def in req.get('required_courses', []):
        if isinstance(course_def, dict):
            ...
            code = course_def; name = code; dcr = 3
        rid = _norm_code(code)
        c = find([rid])
        note = ''
        if course_def.get('also_satisfies'):
            sats = course_def['also_satisfies']
            if 'F4 SI' in sats or 'SI' in sats: note = ' (SI)'
            elif any('WI' in x for x in sats):   note = ' (WI)'
        mr.append({'id': rid, 'label': f"{code} {name}", 'status': status_of(c),
                   'course': c, 'dcr': dcr, 'note': note})

    # Standalone core (Engineering Management 22-23)
    for course_def in req.get('standalone_core', []):
        code = course_def['code']; dcr = course_def.get('credits', 3)
        rid = _norm_code(code)
        c = find([rid])
        mr.append({'id': rid, 'label': code, 'status': status_of(c),
                   'course': c, 'dcr': dcr, 'note': ''})
    ...
    for key in req:
        if key.startswith('choose_one'):
            ...
            if isinstance(opts_raw, list):
                ...
                c = find(nids)
                label = f"Choose one of: {', '.join(opts_raw[:3])}{'...' if len(opts_raw) > 3 else ''}"
                mr.append({'id': 'CHOOSE_ONE', 'label': label, 'status': status_of(c),
                           'course': c, 'dcr': 3, 'note': ''})

    # Electives
    elecs = []; elecs_ip = []; ehrs = 0; ehrs_ip = 0
    elec_required_hrs = 0; elec_opts = []
    ...
    if elec_def and isinstance(elec_def, dict):
        ...
        for opt in choose_from:
            rid = _norm_code(opt)
            c2 = cm.get(rid)
            if c2 and done(c2):   elecs.append(c2);    ehrs    += c2['cr']
            elif c2 and (ip(c2) or sched(c2)): elecs_ip.append(c2); ehrs_ip += c2['cr']
    ...
    # ── GPA ───────────────────────────────────────────────────────────────────
    gp_map = {'A': 4.0, 'A-': 3.7, 'B+': 3.3, 'B': 3.0, 'B-': 2.7,
    ...
    gpa_hrs = 0; qp = 0.0; mgpa_hrs = 0; mqp = 0.0
    earned = 0; ip_hrs = 0
    for c in courses:
        g = c['grade'].upper()
        if done(c) and g not in ('', 'T', 'CR'):
            pts = gp_map.get(g, 0.0)
            gpa_hrs += c['cr']; qp += c['cr'] * pts
            if _norm_code(c['raw']) in major_codes:
                mgpa_hrs += c['cr']; mqp += c['cr'] * pts
    ...
    return {
        ...
        'gpa_o': gpa_o, 'gpa_m': gpa_m,
        'earned': earned, 'ip_hrs': ip_hrs, 'proj': proj,
        'gpa_hrs': gpa_hrs, 'qp': round(qp, 1),
        'la': la, 'bc': bc, 'mr': mr,
        'elecs': elecs, 'elecs_ip': elecs_ip,
        'ehrs': ehrs, 'ehrs_ip': ehrs_ip,
...
def build(res, student_name, major_label, out, exceptions=''):
    ...
    for r in res.get('mr', []):
        note = r.get('note', '')
        if ' (SI)' in note: notes_parts.append(f"{r['id'].replace('_','-')} = SI")
        if ' (WI)' in note: notes_parts.append(f"{r['id'].replace('_','-')} = WI")
    ...
    if 'major_subsections' not in res:
        ...
        if bc_rows:
            res['major_subsections'].append((f"Business Core Requirements ({core_hrs} hrs)", bc_rows))
        if mr_rows:
