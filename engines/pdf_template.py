...
def cr_disp(c, fallback=''):
    if not c: return str(fallback)
    if done(c) or xfer(c): return str(c['cr'])
    if ip(c) or sched(c):  return f"({c['cr']})"
    return str(fallback)
...
def build(res, student_name, major_label, out, exceptions=''):
    ...
    bc_ok_pend = all(r['status'] in ('Satisfied', 'Current', 'Scheduled') for r in res.get('bc', []))
    ...
    side = Table([[met_t, Spacer(gap_w, 1), elig_t]],
                 colWidths=[met_total, gap_w, elig_total])
    ...
    if exceptions and exceptions.strip():
        ...
        if exc_lines:
            ...
            exc_row = Table([[Paragraph(
                f"<b>Advisor Exceptions Applied:</b>  {exc_text}", P['note'])]], colWidths=[CW])
    ...
    if elec_required_hrs > 0:
        elec_label = res.get('elec_section_label', f"Electives — {elec_required_hrs} hrs required  (earned: {ehrs} hrs)")
        ...
        if not elecs and not elecs_ip:
            story.append(mj_row('', f'Electives needed — {elec_required_hrs} hrs required',
                                 'Not Satisfied', str(elec_required_hrs), '', 0))
            ...
            if remaining:
                story.append(mj_row('', f"Additional electives needed from: {' / '.join(remaining)}",
                                     'Not Satisfied', str(elec_required_hrs - ehrs - ehrs_ip), '', row_i))
    ...
    def term_str(c):
        ...
        s = c['status']
        if s == 'current':      return f"Spring, {res.get('current_term_label','2025-26')}"
    ...
    for i, c in enumerate(all_c):
        ...
        row = Table([[
            ...
            Paragraph(c['grade'] if c['grade'] and c['grade'] != 'T' else
    ...
    for r in res['la']:
        ...
        if r['status'] == 'Not Satisfied':
            if area == 'W4':
                ...
                la_miss.append(('WI Writing Intensive #2 — at least one must be upper-division (3000+)', 'See advisor'))
            else:
    ...
    if elec_required_hrs > 0 and ehrs + ehrs_ip < elec_required_hrs:
        ...
        maj_miss.append((f"Electives — {hrs_still_needed} hrs still needed",
