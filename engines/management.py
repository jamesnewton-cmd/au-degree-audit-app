...
ELEC_OPTS = ['BSNS_3100','BSNS_3240','BSNS_3510','BSNS_4050',
             'BSNS_4120','BSNS_4240','BSNS_4310','BSNS_4310_23']
...
LA_ROWS = [
    ...
    ('W8',None,'W8 Experiential Ways of Knowing',
     ['BSNS_1050','BSNS_2550','BSNS_4240','BSNS_4310','BSNS_4500','BSNS_4800'], 2),
...
def audit(courses):
    ...
    for _,_,opts,_ in MGMT_REQ: major_codes.update([norm(o) for o in opts])
...
def build(res, student_name, major_label, out):
    ...
    for i, r in enumerate(res['mr']):
        ...
        code=c['raw'] if c else r['id'].replace('_','-')
        note=''
        if r['id']=='BSNS_4480': note=' (SI)'
        if r['id']=='BSNS_3120': note=' (WI #1 · W7 Liberal Arts)'
        story.append(mj_row(code, r['label']+note, r['status'],
                             cr_disp(c,r['dcr']), grade_disp(c), i))

    # Management Electives
    story.append(sub_section_row(f"Management Electives — 6 hrs required  (earned: {res['ehrs']} hrs)"))
    row_i = 0
    if res['elecs']:
        for c in res['elecs']:
    ...
    if not res['elecs'] and not res['elecs_ip']:
        story.append(mj_row('', 'Electives needed from: BSNS 3100/3240/3510/4050/4120/4240/4310',
                             'Not Satisfied', '6', '', 0))
    elif res['ehrs'] + res['ehrs_ip'] < 6:
        enrolled_codes = {c['raw'].upper() for c in res['elecs']+res['elecs_ip']}
        all_opts = ['BSNS-3100','BSNS-3240','BSNS-3510','BSNS-4050','BSNS-4120','BSNS-4240','BSNS-4310']
    ...
    if res['ehrs'] + res['ehrs_ip'] < 6:
        ...
        maj_miss.append((f"Management Electives — {hrs_still_needed} hrs still needed",
...
if __name__ == '__main__':
    ...
    res     = audit(courses)
    build(res, 'Jenna Miller', 'Management', '/mnt/user-data/outputs/Jenna_Miller_Management_Audit_Final.pdf')
