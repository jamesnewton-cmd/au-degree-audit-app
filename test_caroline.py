"""
Test script: Caroline Hill — Elementary Education (2022-23)
"""
...
# ── 1. Parse CSV ──────────────────────────────────────────────────────────────
raw_courses = sm.parse_csv("/home/ubuntu/upload/CarolineHill-ElementaryEd-22-23.csv")

# ── 2. Load program requirements ──────────────────────────────────────────────
major = "elementary_education"
...
# ── 4. Check ENGL-3590 and ENGL-1400 ─────────────────────────────────────────
print("\n=== ENGL courses on transcript ===")
for c in raw_courses:
    if 'ENGL' in c['code']:
        print(f"  {c['code']:30s} {c['cr']:4.1f} cr  [{c['status']:15s}]  {c['grade']}")

# ── 5. Check PETE-3710 ────────────────────────────────────────────────────────
print("\n=== PETE courses on transcript ===")
for c in raw_courses:
    if 'PETE' in c['code']:
        print(f"  {c['code']:30s} {c['cr']:4.1f} cr  [{c['status']:15s}]  {c['grade']}")

# ── 6. Check EDUC-4120 credits ────────────────────────────────────────────────
print("\n=== EDUC-4120 on transcript ===")
for c in raw_courses:
    if '4120' in c['code']:
        print(f"  {c['code']:30s} {c['cr']:4.1f} cr  [{c['status']:15s}]  {c['grade']}")
